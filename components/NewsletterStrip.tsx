...
export default function NewsletterStrip() {
  ...
  return (
    <section ref={ref} className="relative py-20 px-6 overflow-hidden bg-[#050505]">
      ...
      <div className="relative max-w-3xl mx-auto text-center">
        ...
        >
          ...
          <p className="text-[#C9A84C] text-[11px] tracking-[0.4em] uppercase mb-3">Stay in the Know</p>
        ...
        >
          {status === "ok" ? (
            <motion.div
              ...
              className="inline-flex items-center gap-3 px-8 py-4 border border-[#C9A84C]/30 bg-[#C9A84C]/8"
            >
              <div className="w-6 h-6 border border-[#C9A84C]/40 flex items-center justify-center flex-shrink-0">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-xl mx-auto">
              <input
                ...
                className="flex-1 bg-[#0a0a0a] px-5 py-3.5 text-sm text-white placeholder-[#333] focus:outline-none transition-colors"
                style={{ border: `1px solid ${focused === "name" ? "rgba(201,168,76,0.5)" : "#1a1a1a"}` }}
              />
              <input
                type="email"
                required
                ...
                onBlur={() => setFocused(null)}
                className="flex-1 bg-[#0a0a0a] px-5 py-3.5 text-sm text-white placeholder-[#333] focus:outline-none transition-colors"
                style={{ border: `1px solid ${focused === "email" ? "rgba(201,168,76,0.5)" : "#1a1a1a"}` }}
              />
              <button
                type="submit"
                disabled={status === "loading"}
                className="cursor-pointer px-8 py-3.5 bg-[#C9A84C] text-black font-semibold text-xs uppercase tracking-widest hover:bg-[#FFD700] transition-all duration-200 disabled:opacity-50 whitespace-nowrap"
              >
        ...
        >
          {["Exclusive offers", "New service alerts", "Style tips", "Seasonal promotions"].map((perk) => (
            <div key={perk} className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] text-[#444]">{perk}</span>
            </div>
          ))}
        </motion.div>
      </div>
