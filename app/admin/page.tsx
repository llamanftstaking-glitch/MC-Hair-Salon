  ...
  return (
    <div className="min-h-screen bg-black pt-20">
      ...
      <div className="border-b border-[#2a2a2a] bg-[#050505]">
        <div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between flex-wrap gap-4">
          ...
          <button onClick={() => { fetchBookings(); fetchSubscribers(); }}
            className="flex items-center gap-2 border border-[#2a2a2a] text-[#666] px-4 py-2 text-sm hover:border-[#C9A84C] hover:text-[#C9A84C] transition-all cursor-pointer">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {/* Tabs */}
        <div className="flex gap-0 mb-8 border-b border-[#2a2a2a] overflow-x-auto">
          {tabs.map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`flex items-center gap-2 px-6 py-3 text-sm uppercase tracking-widest font-semibold transition-all cursor-pointer border-b-2 -mb-px whitespace-nowrap ${
                tab === t.id ? "border-[#C9A84C] text-[#C9A84C]" : "border-transparent text-[#555] hover:text-[#a89070]"
              }`}>
              {t.icon} {t.label}
            </button>
          ))}
              ...
              {(["all", "pending", "confirmed", "cancelled"] as const).map(f => (
                <button key={f} onClick={() => setFilter(f)}
                  className={`px-4 py-1.5 text-xs uppercase tracking-widest cursor-pointer transition-all ${
                    filter === f ? "gold-gradient-bg text-black font-bold" : "border border-[#2a2a2a] text-[#666] hover:border-[#C9A84C]"
                  }`}>
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 text-sm mb-2">
                          <span className="text-[#666] truncate">{booking.email}</span>
                          <span className="text-[#666]">{booking.phone}</span>
                          <span className="text-[#C9A84C] font-medium">{booking.date} · {booking.time}</span>
                          <span className="text-[#a89070]">{booking.stylist || "No preference"}</span>
                        </div>
                        <p className="text-[#555] text-sm">{booking.service}</p>
                        {booking.notes && <p className="text-[#444] text-xs mt-1 italic">"{booking.notes}"</p>}
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        {booking.status === "confirmed" && (
                          <button onClick={() => sendConfirmationEmail(booking.id)}
                            className="w-9 h-9 flex items-center justify-center border border-[#C9A84C]/30 text-[#C9A84C] hover:bg-[#C9A84C]/10 transition-colors cursor-pointer" title="Resend confirmation email">
                            <Send size={14} />
                          </button>
                        )}
                        ...
                        <button onClick={() => deleteBooking(booking.id)}
                          className="w-9 h-9 flex items-center justify-center border border-[#2a2a2a] text-[#555] hover:border-red-500/50 hover:text-red-400 transition-all cursor-pointer">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </div>
                    ...
                    <div key={client.email} className="luxury-card p-6">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
                        <div>
                          ...
                          <p className="text-[#666] text-sm">{client.email}</p>
                          <p className="text-[#666] text-sm">{client.phone}</p>
                        </div>
                      ...
                      <div className="border-t border-[#1a1a1a] pt-4 space-y-2">
                        <p className="text-[#C9A84C] text-xs uppercase tracking-widest font-semibold mb-3">Appointment History</p>
                        {visits.map(v => (
                          <div key={v.id} className="flex flex-col sm:flex-row sm:items-center justify-between text-sm gap-1">
                            <div>
                              <span className="text-[#a89070]">{v.date} at {v.time}</span>
                              <span className="text-[#444] mx-2">·</span>
                              <span className="text-[#666]">{v.service.split("–")[1]?.trim() || v.service}</span>
                              {v.stylist && <><span className="text-[#444] mx-2">·</span><span className="text-[#555]">{v.stylist}</span></>}
                            </div>
            ...
            <div className="space-y-6">
              <div className="luxury-card p-6">
                <h3 className="text-white font-semibold text-lg mb-6 flex items-center gap-2">
                  <Send size={18} className="text-[#C9A84C]" /> Compose Newsletter
                </h3>
                <div className="space-y-4">
                  <div>
                    <label className="block text-[#C9A84C] text-xs uppercase tracking-widest font-semibold mb-2">Subject</label>
                    <input
                      ...
                      className="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white px-4 py-3 text-sm focus:outline-none focus:border-[#C9A84C] transition-colors placeholder-[#444]"
                    />
                  </div>
                  <div>
                    <label className="block text-[#C9A84C] text-xs uppercase tracking-widest font-semibold mb-2">Message</label>
                    <textarea
                      ...
                      className="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white px-4 py-3 text-sm focus:outline-none focus:border-[#C9A84C] transition-colors placeholder-[#444] resize-none"
                    />
                  </div>
                  <div className="flex items-center justify-between pt-2">
                    <p className="text-[#555] text-xs">Sending to <span className="text-[#C9A84C]">{activeSubscribers}</span> subscriber{activeSubscribers !== 1 ? "s" : ""}</p>
              ...
              <div className="luxury-card p-6">
                <h3 className="text-white font-semibold mb-4 flex items-center gap-2">
                  <Plus size={16} className="text-[#C9A84C]" /> Add Subscriber
                </h3>
                <div className="flex flex-col gap-3">
                  <input type="text" value={newName} onChange={e => setNewName(e.target.value)} placeholder="Name (optional)"
                    className="w-full bg-[#0a0a0a] border border-[#2a2a2a] text-white px-4 py-2.5 text-sm focus:outline-none focus:border-[#C9A84C] transition-colors placeholder-[#444]" />
            ...
            <div className="luxury-card p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-white font-semibold flex items-center gap-2">
                  <Users size={16} className="text-[#C9A84C]" /> Subscribers
                  <span className="text-[#C9A84C] text-sm font-normal">({activeSubscribers} active)</span>
                </h3>
              </div>
              {subscribers.length === 0 ? (
                ...
                <div className="space-y-2 max-h-[600px] overflow-y-auto pr-1">
                  {subscribers.map(sub => (
                    <div key={sub.id} className={`flex items-center justify-between p-3 border ${sub.active ? "border-[#2a2a2a]" : "border-[#1a1a1a] opacity-40"}`}>
                      <div>
                        {sub.name && <p className="text-white text-sm font-medium">{sub.name}</p>}
                        <p className="text-[#666] text-sm">{sub.email}</p>
                        <p className="text-[#333] text-xs mt-0.5">{new Date(sub.subscribedAt).toLocaleDateString()}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className={`text-xs px-2 py-0.5 border ${sub.active ? "text-green-400 border-green-400/30 bg-green-400/10" : "text-[#555] border-[#2a2a2a]"}`}>
                          {sub.active ? "active" : "off"}
                        </span>
                        <button onClick={() => removeSubscriber(sub.id)}
                          className="w-7 h-7 flex items-center justify-center text-[#444] hover:text-red-400 transition-colors cursor-pointer">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </div>
        ...
        {tab === "reports" && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="luxury-card p-6">
                <h3 className="text-white font-semibold mb-6 flex items-center gap-2"><BarChart2 size={16} className="text-[#C9A84C]" /> Top Services</h3>
                {topServices.length === 0 ? <p className="text-[#555] text-sm">No data yet</p> : (
                  <div className="space-y-4">
                    {topServices.map(([name, count]) => (
                      <div key={name}>
                        <div className="flex justify-between text-sm mb-1.5">
                          <span className="text-[#a89070] truncate mr-4">{name}</span>
                          <span className="text-[#C9A84C] shrink-0">{count} booking{count !== 1 ? "s" : ""}</span>
                        </div>
              ...
              <div className="luxury-card p-6">
                <h3 className="text-white font-semibold mb-6 flex items-center gap-2"><Users size={16} className="text-[#C9A84C]" /> Bookings by Stylist</h3>
                {Object.keys(stylistCount).length === 0 ? <p className="text-[#555] text-sm">No data yet</p> : (
                  <div className="space-y-4">
                    {Object.entries(stylistCount).sort((a, b) => b[1] - a[1]).map(([stylist, count]) => (
                      <div key={stylist}>
                        <div className="flex justify-between text-sm mb-1.5">
                          <span className="text-[#a89070]">{stylist}</span>
                          <span className="text-[#C9A84C]">{count} booking{count !== 1 ? "s" : ""}</span>
                        </div>
                        <div className="h-1.5 bg-[#1a1a1a] rounded-full">
